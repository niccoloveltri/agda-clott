...
In addition, this grammar does not allow the possibility of defining nested inductive types.
...
\Ar{Γ}. This is analogous to Atkey and McBride's side condition
requiring the universally quantified clock variable to not appear freely
...
formers. We only show the ones associated to the
\IC{□} modality here. The rules state that \IC{box} and \IC{unbox} are each
other's inverses.
%up to \AD{∼}.
\AgdaHide{
\begin{code}
