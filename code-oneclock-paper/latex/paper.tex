...
\cite{bizjak2016guarded} and ticks \cite{BahrGM17}. We plan to extend
...
normalization. In future work, we plan to investigate the metatheory
and the dynamic behaviour of \GTT, for example by formalizing a  type
...
Finally, we want to understand weather it is possible to reduce sized types to guarded
recursion.  This does not seems directly doable, since sizes are not uniquely employed with coinductive types as a
tool for easing the productivity check of corecursive
definitions, but they are also used as auxiliary arguments of inductive
types for facilitating the termination check of recursive
definitions. It seems that the existing type theories for guarded
recursion are not capable of capturing this dual aspect of sizes. 
