...
record Fun {ℓ} (t t' : Ty ℓ) (κ : Clock) : Set ℓ where
  constructor fun
  open Ty t
  open Ty t' renaming (A to A'; next to next')
  field
    f : (α : Tick= κ) → A α → A' α
    nextᶠ : (α : Tick= κ) (β : Tick= α) (x : A α)
           → next' α β (f α x) ≡ f β (next α β x) 
...
Fun-eq : ∀ {ℓ} {t t' : Ty ℓ} {κ : Clock}
  → {g g' : Fun t t' κ}
  → Fun.f g ≡ Fun.f g' → g ≡ g'
Fun-eq {κ = κ}{fun f p}{fun .f q} refl =
  cong (fun f) (funext (λ _ → funext (λ { _ → funext (λ _ → uip) })))

_⇒_ : ∀ {ℓ} (t t' : Ty ℓ) → Ty ℓ
t ⇒ t' = ty (Fun t t') (λ _ _ → id) (λ _ _ _ _ → refl) (λ _ _ → refl)
...
app : ∀{ℓ} {t t' : Ty ℓ}
  → Tm ℓ (t ⇒ t')
  → Tm ℓ t → Tm ℓ t'
app {t = ty A next-A _ _} {ty B next-B _ _} (tm gn nextᵀᵐ-gn) (tm x nextᵀᵐ-x) =
  tm (λ κ → g κ κ (x κ))
     (λ κ α →     
        begin
       next-B κ α (g κ κ (x κ))
         ≡⟨ nextᶠ-g κ κ α (x κ) ⟩ 
       g κ α (next-A κ α (x κ))
         ≡⟨ cong (g κ α) (nextᵀᵐ-x κ α) ⟩
       g κ α (x α)
         ≡⟨ cong (λ z → Fun.f z α (x α)) (nextᵀᵐ-gn κ α) ⟩
       g α α (x α)
         ∎)
  where
    g : (κ : Clock) (α : Tick= κ) → A α → B α
    g κ = Fun.f (gn κ)

    nextᶠ-g : (κ : Clock) (α : Tick= κ) (β : Tick= α) (y : A α)
      → next-B α β (g κ α y) ≡ g κ β (next-A α β y)
    nextᶠ-g κ = Fun.nextᶠ (gn κ)
...
{-
nextPi : ∀ {ℓ} (c : ClTy ℓ) (t : Ty ℓ c)
  → (κ : Size) (α : Size< κ) → Pi c t κ → Pi c t α
nextPi c (ty A nA aA) κ α (pi f nf) = pi (λ β x → {!f β x!}) {!!}
-}
