...
\begin{abstract}
  ...
  achieved using coinductive types. Crucially, recursively defined
  programs of coinductive types are required to be productive. Proof
  assistants such as Agda and Coq traditionally employ strict syntactic
  productivity checks, which commonly make programming with coinductive types
  convoluted. One way to overcome this issue is by encoding productivity at
...
%%  finite unfoldings are guaranteed to produce . 
  ...
  types. First we formalize the syntax of a simple type theory for
  guarded recursion, corresponding to a variant of Atkey and McBride's
  type system for productive coprogramming
  \cite{atkey2013productive}. Then we give a denotational semantics
  using presheaves over the preorder of sizes. Sized types are
  fundamentally employed in the interpretation of the characteristic
