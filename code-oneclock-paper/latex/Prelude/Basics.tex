...
We make extensive use of record types. These are like iterated $\Sigma$-types, in which each component, called field, has been given a name. A field of a record type can be accessed by function application. For example, given a record type \F{R} containing a field \Fi{f}
