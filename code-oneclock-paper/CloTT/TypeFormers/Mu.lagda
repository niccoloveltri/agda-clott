...
\IC{κ}. This try does not work, since \F{μObj} \AB{P} is a sized type
while \F{sem-eval} \AB{P} expects a semantic type as its input.
...
This is the case where we would like to make a recursive call to \F{μObj} applied to the original code \AB{P}, but this is unavailable at this point.
We can solve the issue by introducing an auxiliary inductive type family \F{μObj'}, which depends on two codes instead of one.
