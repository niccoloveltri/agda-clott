...
\AgdaHide{
\begin{code}
refl-def-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {t : Tm Γ A}
  → def-eq Γ A t t
refl-def-eq {∅} γ = refl
refl-def-eq {κ} i γ = refl

sym-def-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {s t : Tm Γ A}
  → def-eq Γ A s t → def-eq Γ A t s
sym-def-eq {∅} p γ = sym (p γ)
sym-def-eq {κ} p i γ = sym (p i γ)

trans-def-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {s t u : Tm Γ A}
  → def-eq Γ A s t → def-eq Γ A t u → def-eq Γ A s u
trans-def-eq {∅} p q γ = trans (p γ) (q γ)
trans-def-eq {κ} p q i γ = trans (p i γ) (q i γ)

eq-to-def-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {s t : Tm Γ A}
  → s ≡ t → def-eq Γ A s t
eq-to-def-eq refl = refl-def-eq

def-eq-to-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {s t : Tm Γ A}
  → def-eq Γ A s t → s ≡ t
def-eq-to-eq {∅} p = funext p
def-eq-to-eq {κ} p = NatTrans-eq p

eq-to-def-eq-to-eq : {b : ClockContext} {Γ : Ctx b} {A : Ty b} {s t : Tm Γ A}
...
\begin{code}
subst-eq : {b : ClockContext} (Γ Γ' : Ctx b) (s t : sem-subst Γ Γ') → Set
...
\AgdaHide{
\begin{code}
refl-subst-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s : sem-subst Γ Γ'}
  → subst-eq Γ Γ' s s
refl-subst-eq {∅} x = refl
refl-subst-eq {κ} i x = refl

sym-subst-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t : sem-subst Γ Γ'}
  → subst-eq Γ Γ' s t → subst-eq Γ Γ' t s
sym-subst-eq {∅} p γ = sym (p γ)
sym-subst-eq {κ} p i γ = sym (p i γ)

trans-subst-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t u : sem-subst Γ Γ'}
...
eq-to-subst-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t : sem-subst Γ Γ'}
  → s ≡ t → subst-eq Γ Γ' s t
eq-to-subst-eq refl = refl-subst-eq

subst-eq-to-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t : sem-subst Γ Γ'}
  → subst-eq Γ Γ' s t → s ≡ t
subst-eq-to-eq {∅} p = funext p
subst-eq-to-eq {κ} p = NatTrans-eq p

subst-eq-to-eq-to-subst-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t : sem-subst Γ Γ'}
  → (p : s ≡ t)
  → subst-eq-to-eq(eq-to-subst-eq p) ≡ p
subst-eq-to-eq-to-subst-eq p = uip

eq-to-subst-eq-to-eq : {b : ClockContext} {Γ Γ' : Ctx b} {s t : sem-subst Γ Γ'}
  → (p : subst-eq Γ Γ' s t)
