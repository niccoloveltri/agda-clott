...
-- Introduction rule
pair : {b : Bool} (Γ : Ctx b) (A B : Ty b) (x : Tm b Γ A) (y : Tm b Γ B) → Tm b Γ (A ⊗ B)
...
-- Elimination rules
pr₁ : {b : Bool} (Γ : Ctx b) (A B : Ty b) → Tm b Γ (A ⊗ B) → Tm b Γ A
pr₁ {set} Γ A B x t = proj₁ (x t)
proj₁ (pr₁ {tot} Γ A B (x , p)) i t = proj₁ (x i t)
proj₂ (pr₁ {tot} Γ A B (x , p)) i j t = cong proj₁ (p i j t)

pr₂ : {b : Bool} (Γ : Ctx b) (A B : Ty b) → Tm b Γ (A ⊗ B) → Tm b Γ B
pr₂ {set} Γ A B x t = proj₂ (x t)
proj₁ (pr₂ {tot} Γ A B (x , p)) i t = proj₂ (x i t)
proj₂ (pr₂ {tot} Γ A B (x , p)) i j t = cong proj₂ (p i j t)

-- Computation rules
pr₁-pair : {b : Bool} (Γ : Ctx b) (A B : Ty b) (x : Tm b Γ A) (y : Tm b Γ B)
  → def-eq Γ A
...
-- Eta rule
prod-eta : {b : Bool} (Γ : Ctx b) (A B : Ty b) (x : Tm b Γ (A ⊗ B))
  → def-eq Γ (A ⊗ B)
